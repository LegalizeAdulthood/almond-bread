...
void GIFEncode (FILE* fp, int GWidth, int GHeight, int GInterlace, int Background, int BitsPerPixel, int Red[], int Green[], int Blue[], ifunptr GetPixel);
...
ifunptr GetPixel;
{
	...
	fwrite(loaded!=GIF?"GIF89a":"GIF87a",1,6,fp);
	...
	if(loaded!=GIF)
	  {
	    unsigned char *p=(unsigned char *)fi;
	    
	    fputc(0x21,fp);
	    fputc(0xff,fp);
	    fputc(11,fp);
	    fwrite("fractint001",1,11,fp);
	    for(i=0; i+255<FRACTAL_INFO_SIZE; i+=255)
	      {
		fputc(255,fp);
		fwrite(p+i,255,1,fp);
	      }
	    fputc(FRACTAL_INFO_SIZE-i,fp);
	    fwrite(p+i,FRACTAL_INFO_SIZE-i,1,fp);
	    fputc(0,fp);
	  }
...
writeerr()
{
        gif_error( "error writing output file");
}
